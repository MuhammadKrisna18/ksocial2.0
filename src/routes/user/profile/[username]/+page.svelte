			...
			<div class="mt-4 sm:mt-0 w-full sm:w-auto">
				...
				{:else}
					<form method="POST" action="?/follow" use:enhance={() => {
						...
						<button 
							type="submit" 
							disabled={followStatus !== 'none' || isFollowingState}
							class="w-full sm:w-auto inline-flex justify-center items-center px-8 py-3 rounded-xl {followStatus === 'none' ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-500/20 active:scale-95 text-white shadow-md' : 'bg-slate-200 dark:bg-slate-800 text-slate-500 cursor-not-allowed'} font-bold transition-all"
						>
							{#if isFollowingState}
